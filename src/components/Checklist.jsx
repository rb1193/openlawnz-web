...
export default function Checklist({ id, items }) {
  const [checklist, setChecklist] = useState(
    items.map((item) => {
      return Array(item.items.length).fill(CHECKLIST_EMPTY)
    })
  )

  const toggleCheck = (parentIndex, index) => {
    setChecklist(produce(checklist, newState => {
    ...
    return (
      <article key={`checklist_item_${index}`} className="checklist-item">
        <header className="checklist-item-header">
          <strong id={titleId} className="checklist-item-title">{item.title}</strong>
        </header>
    ...
    return (
      <li key={`checklist_item_${parentIndex}_subitem_${index}`} className="checklist-sub-item">
        <div className="checklist-sub-item-control">
          <button className="checklist-item-checkbox" onClick={() => {toggleCheck(parentIndex, index)}}>
            {checklist[parentIndex][index] === CHECKLIST_CHECKED && (
              <img src="/assets/check.svg" alt="checked" />
            )}
          </button>
        </div>
        <div className="checklist-sub-item-content">
          <strong className="checklist-sub-item-title">{item.title}</strong>
          <p>{item.content}</p>
        </div>
      </li>
    )
